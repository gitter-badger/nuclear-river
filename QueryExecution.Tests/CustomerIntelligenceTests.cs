...
namespace NuClear.AdvancedSearch.QueryExecution.Tests
{
    ...
    public sealed class CustomerIntelligenceTests : QueryExecutionBaseFixture
    {
        ...
        [TestCase("Firm", "$filter=Territories/any(it: (it/TerritoryId eq 123))", Result = "Firm[].Where($it => $it.Territories.Any(it => (it.TerritoryId == 123)))", Description = "Поиск по территории.")]
