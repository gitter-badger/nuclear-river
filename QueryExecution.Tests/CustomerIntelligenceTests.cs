...
namespace NuClear.AdvancedSearch.QueryExecution.Tests
{
    ...
    public sealed class CustomerIntelligenceTests : QueryExecutionBaseFixture
    {
        ...
        [TestCase("$filter=Categories1/any(x:x/Category/Id eq @id)&@id=123", Result = "Firm[].Where($it => $it.Categories1.Any(x => (x.Category.Id == Convert(123))))", Description = "Поиск по рубрике 1-го уровня.")]
        [TestCase("$filter=Categories2/any(x:x/Category/Id eq @id)&@id=123", Result = "Firm[].Where($it => $it.Categories2.Any(x => (x.Category.Id == Convert(123))))", Description = "Поиск по рубрике 2-го уровня.")]
        [TestCase("$filter=Categories3/any(x:x/Category/Id eq @id)&@id=123", Result = "Firm[].Where($it => $it.Categories3.Any(x => (x.Category.Id == Convert(123))))", Description = "Поиск по рубрике 3-го уровня.")]
