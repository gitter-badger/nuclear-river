...
namespace NuClear.AdvancedSearch.OData.Query.Tests
{
    ...
    {
        public static IEdmModel AddClrAnnotations(this IEdmModel model, IReadOnlyList<Type> clrTypes)
        {
            var tuples = model.SchemaElements.OfType<IEdmTerm>().Join(clrTypes, x => x.Name, x => x.Name, Tuple.Create);
        ...
        {
            var tuples = edmType.DeclaredProperties.Join(clrType.GetProperties(), x => x.Name, x => x.Name, Tuple.Create);
            foreach (var tuple in tuples)
            {
