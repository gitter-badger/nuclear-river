    ...
    public sealed class FactsTransformation
    {
        private static readonly Dictionary<Type, Func<IFactsContext, IEnumerable<long>, IQueryable>> Queries = 
            new Dictionary<Type, Func<IFactsContext, IEnumerable<long>, IQueryable>>
                {
                    { typeof(Fact.Account), (context, ids) => context.Accounts.Where(x => ids.Contains(x.Id)) },
                    { typeof(Fact.CategoryFirmAddress), (context, ids) => context.CategoryFirmAddresses.Where(x => ids.Contains(x.Id)) },
                    { typeof(Fact.CategoryOrganizationUnit), (context, ids) => context.CategoryOrganizationUnits.Where(x => ids.Contains(x.Id)) },
                    { typeof(Fact.Client), (context, ids) => context.Clients.Where(x => ids.Contains(x.Id)) },
                    { typeof(Fact.Contact), (context, ids) => context.Contacts.Where(x => ids.Contains(x.Id)) },
                    { typeof(Fact.Firm), (context, ids) => context.Firms.Where(x => ids.Contains(x.Id)) },
                    { typeof(Fact.FirmAddress), (context, ids) => context.FirmAddresses.Where(x => ids.Contains(x.Id)) },
                    { typeof(Fact.FirmContact), (context, ids) => context.FirmContacts.Where(x => ids.Contains(x.Id)) },
                    { typeof(Fact.LegalPerson), (context, ids) => context.LegalPersons.Where(x => ids.Contains(x.Id)) },
                    { typeof(Fact.Order), (context, ids) => context.Orders.Where(x => ids.Contains(x.Id)) },
                };
